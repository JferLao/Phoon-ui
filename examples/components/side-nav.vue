	...
	<div class="side-nav" @mouseenter="isFade = false" :class="{ 'is-fade': isFade }" :style="navStyle">
		<ul>
			<li class="nav-item" v-for="(item, key) in data" :key="key">
				<!-- 如果没有路径地址则展开菜单 -->
				<a v-if="!item.path && !item.href" @click="expendMenu">{{ item.name }}</a>
				...
				<template v-if="item.groups">
					<div class="nav-group" v-for="(group, key) in item.groups" :key="key">
						<div class="nav-group__title" @click="expendMenu">{{ group.groupName }}</div>
						<ul class="pure-menu-list">
							<li class="nav-item" v-for="(navItem, key) in group.list" v-show="!navItem.disabled" :key="key">
								<router-link active-class="active" :to="base + navItem.path" exact v-text="navItem.title|| navItem.name"></router-link>
							</li>
						</ul>
					</div>
				</template>
...
export default {
	...
	computed: {
		...
		langConfig() {
			return compoLang.filter((config) => {
